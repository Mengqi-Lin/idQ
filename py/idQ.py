...
def Any_two_columns_contain_I(Q):
    ...
    for col1, col2 in column_combinations:
        ...
        if not contains_identity_submatrix(sub_matrix):
            return False
...
def get_reduced(Q):
    """
    Takes a binary matrix Q (list of lists of 0/1) and returns:
      1. Q_reduced: A reduced, sorted version of Q that:
         - Is sorted lexicographically
         - Has no zero rows
         - Has no duplicate rows
         - Omits any row that can be obtained by the bitwise OR
           of some subset of previously included rows.
      2. mapping: A list of lists. For the i-th row in the original Q,
                  mapping[i] is a list of indices into Q_reduced such that
                  bitwise-OR of those rows from Q_reduced yields Q[i].
      3. original_indices_for_reduced: For each row in Q_reduced, a list of
                  indices from the original Q that exactly match that row.
    """
    # -- 1) Convert each row to a tuple and store original indices for each unique row
    rows = [tuple(row) for row in Q]
    row_to_indices = {}
    for i, r in enumerate(rows):
        row_to_indices.setdefault(r, []).append(i)

    # -- 2) Sort all row-tuples lexicographically
    rows_sorted = sorted(rows)

    # or_dict will map: row_tuple -> list of Q_reduced indices that OR to form row_tuple
    or_dict = {}

    # Determine the number of columns
    n_cols = len(rows_sorted[0]) if rows_sorted else 0
    zero_tuple = tuple([0]*n_cols)

    # We say zero_tuple can be formed by an empty subset
    or_dict[zero_tuple] = []

    # Q_reduced and a structure to track the original indices for each row in Q_reduced
    Q_reduced_tuples = []
    original_indices_for_reduced = []

    # A small helper for bitwise OR on tuples
    def bitwise_or(a, b):
        return tuple(x or y for x, y in zip(a, b))

    # -- 3) Build Q_reduced by scanning sorted rows
    for r in rows_sorted:
        # Skip zero rows
        if r == zero_tuple:
            continue

        # If r is not already generated by an OR-combination of smaller rows, add it
        if r not in or_dict:
            # Add r to Q_reduced
            Q_reduced_tuples.append(r)
            # The index of this newly added row in Q_reduced
            new_index = len(Q_reduced_tuples) - 1

            # Record the original indices that matched exactly this row
            original_indices_for_reduced.append(row_to_indices[r])

            # Update or_dict so we can form new OR-combinations
            existing_rows = list(or_dict.keys())
            for existing_row in existing_rows:
                or_result = bitwise_or(existing_row, r)
                if or_result not in or_dict:
                    # or_dict[existing_row] + [new_index] is how we form or_result
                    or_dict[or_result] = or_dict[existing_row] + [new_index]

    # -- 4) Build the mapping for each row in the original Q
    # mapping[i] will be the subset of Q_reduced indices that forms Q[i] by OR
    mapping = []
    for row_tuple in rows:
        mapping.append(or_dict[row_tuple])

    # Convert Q_reduced back to list-of-lists
    Q_reduced = [list(tup) for tup in Q_reduced_tuples]

    return Q_reduced, mapping, original_indices_for_reduced



def get_original(Q_reduced, mapping):
    """
    Given Q_reduced (list of lists of 0/1) and a mapping (list of lists of indices),
    reconstruct the original Q matrix by bitwise OR-ing the indicated rows from Q_reduced.
    """
    if not Q_reduced:
        # If there's nothing in Q_reduced, every row in the original must be zero
        # We'll assume 0 columns for safety
        return [[0]*0 for _ in mapping]

    n_cols = len(Q_reduced[0])
    original_Q = []

    for row_map in mapping:
        row = [0]*n_cols
        for idx in row_map:
            for c in range(n_cols):
                row[c] = row[c] or Q_reduced[idx][c]
        original_Q.append(row)
...
def identifiability(Q):
    ...
    # Step 1: get Q_reduced
    Q_reduced, mapping, original_idx = get_reduced(Q1)
    ...
    for k in range(Q_reduced.shape[1]):
        if np.all(Q_reduced[:, k] == 0):
            ...
            Q_bar = get_original(Q_reduced_bar, mapping)
            return 0, Q_bar
